# Multi-ToolHead 3D Printer PurpleWhale(PW)


**PW** is a 160mm x 160mm x 300mm custom Multitoolhead 3d printer heavily inspired by the VORON. It is a 100% custom design. This project was made for Fallout. The tool swapping mechanism allows users to print in a variety of colors with minimal waste. The overall goal of this printer was to be able to print in multiple colors with minimal waste. The printer uses Klipper firmware running on the BTT CB2 compute module.

# Why I designed this printer
I created this printer so I could print out figurines in multiple colors instead of having to sand and paint them. I suck at painting so I thought this printer would get rid of that issue. Its mostly so I can print warhammer figurines in multiple colors without having to paint them. 

| CAD | Zine |
|:---:|:---:|
| ![CAD Render](https://github.com/user-attachments/assets/47b7f846-7fa0-418e-8c82-2d5cd105806e) | ![Tool Swapping Mechanism](https://github.com/user-attachments/assets/d19c9245-4959-42f4-bf7a-b7ae525c489c) |

more CAD                         

  <img width="1520" height="722" alt="image" src="https://github.com/user-attachments/assets/82509811-5408-434a-9924-b8e7a9280983" />
  <img width="1520" height="722" alt="image" src="https://github.com/user-attachments/assets/9d6bb2e6-a30e-4061-8248-f8babb43b6ab" />


---
#Printer Wiring Diagram
This is a extremely rudimentary generalization of the wiring. 
<img width="1311" height="901" alt="PrinterSchematic" src="https://github.com/user-attachments/assets/21c64d5f-6900-4278-96eb-1668bb945c33" />

---

## Tool Swapping Mechanism

The printer uses a custom made tool swapping mechanism that uses 3 pogo pins to hold tool heads. Heavily inspired by the stealth changer. 


## Gantry Mechanism

This printer uses a flying COREXY gantry mechanism. The printer can move in the XY direction using only two motors. It moves in the z axis using 4 stepper motors with linear rails and leadscrews.

## Assembly 
To assemble this, you will need a bunch of M3, M4, and M5 screws with their respective nuts. 

## Bill of Materials
A more detailed Bill Of Materials exists as [`BOM.CSV`]((https://github.com/Pnav22/Multi-ToolHead-Printer-PurpleWhale/blob/main/BOM.csv))   


| Item | Quantity | Cost (USD) |
|------|----------|------------|
| BTT Manta M8P V2.0 | 1 | $85 |
| BTT CB2 Compute Module | 1 | $35 |
| BTT EBB36 CAN Toolhead Board | 4 | $100 |
| UTOC-3 USB-CAN Adapter | 1 | $20 |
| Meanwell LRS-750-24 PSU | 1 | $55 |
| 5V Buck Converter | 1 | $8 |
| Wiring Terminal Board | 1 | $12 |
| 2020/2040 Aluminum Extrusion | Assorted | $60 |
| MGN12H Linear Rail | 5 | $65 |
| Gates 2GT Belt + Pulleys | 1 Set | $35 |
| NEMA 17 Stepper Motor | 5 | $55 |
| Leadscrew + Brass Nut | 2 | $25 |
| Idlers + Hardware Kit | 1 Set | $20 |
| TZ V6 Hotend | 4 | $100 |
| Orbiter v2 Extruder | 4 | $140 |
| 5015 Blower Fan | 4 | $24 |
| 3010 Hotend Fan | 4 | $16 |
| Pogo Pin Connector | 4 | $30 |
| Kinematic Coupling Balls + Magnets | 4 Sets | $40 |
| Dock Brackets (Printed) | 4 | $5 |
| Steel Kinematic Ball Dock | 4 | $15 |
| N52 Neodymium Magnets | Assorted | $20 |
| 300x300 Aluminum Bed Plate | 1 | $25 |
| PEI Spring Steel Sheet | 1 | $20 |
| 300x300 Silicone Bed Heater (750W) | 1 | $30 |
| SSR-25DA Solid State Relay | 1 | $12 |
| Printed Parts Filament | 1 Lot | $20 |
| Wiring + Connectors + Crimps | 1 Lot | $25 |
| PTFE-Sleeved Wire Bundle | 2 | $15 |
| Klicky or CR Touch Probe | 1 | $15 |
| Frame Brackets + T-Nuts | 1 Set | $20 |
| Lubricant | 1 | $10 |
| Oil | 1 | $10 |

Total price: $1,117 USD
